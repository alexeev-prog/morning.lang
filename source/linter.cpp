...
Linter::Linter()
    ...
    addRule({"W001",
             ...
             [this](const Exp& exp, std::vector<std::string>& issues)
             {
                 ...
                 if (operators_.count(name) > 0 || name == "func" || name == "var" || name == "const"
    ...
    addRule({"W002",
             ...
             [this](const Exp& exp, std::vector<std::string>& issues)
             {
                 ...
                 const std::string& name = exp.string;
                 if (operators_.count(name) > 0 || !is_valid_identifier(name)) {
                     return;
                 }

                 bool has_uppercase =
                     std::any_of(name.begin(), name.end(), [](char c) { return std::isupper(c); });
                 ...
                 if (head.string == "func" || head.string == "var" || head.string == "const") {
                     ...
                     if (exp.list.size() > 1) {
                         ...
                         if (nameExp.type == ExpType::SYMBOL) {
                             name = nameExp.string;
                         } else if (nameExp.type == ExpType::LIST && !nameExp.list.empty()
...
auto Linter::suggest_snake_case(const std::string& name) const -> std::string {
    ...
    for (size_t i = 0; i < name.size(); ++i) {
        ...
        if (std::isupper(c)) {
            ...
            if (i > 0 && !std::isspace(suggestion.back()) && suggestion.back() != '_') {
                suggestion += '_';
            }
